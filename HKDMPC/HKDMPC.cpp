        ...
        for (int l(0); l < 4; l++)
        {
            ...
            if (!found_next_ctact[l])
            {
                ...
                if (ctact[l] == 0 && ctactn[l] == 1)
                {
                    ...
                    const auto& state_td = opt_problem_data.trajectory_ptrs[i + 1]->Xbar[0].head(12); 
                    Vec3<float> pcom_td = state_td.segment(3,3).template cast<float>();
                    pf_rel_com[l] = pf[l] - pcom_td;
                    vcom_td = state_td.segment(9,3).template cast<float>();
...
void HKDMPCSolver<T>::publish_mpc_cmd()
{
    ...
    for (int l = 0; l < 4; l++)
    {
        ...
        hkd_cmds.foot_placement_rel_com[3 * l] = pf_rel_com[l][0];
        hkd_cmds.foot_placement_rel_com[3 * l + 1] = pf_rel_com[l][1];
        hkd_cmds.foot_placement_rel_com[3 * l + 2] = pf_rel_com[l][2];
    }
    for (int j = 0; j < 3; j++){
        hkd_cmds.vcom_td[j] = vcom_td[j];
    }
